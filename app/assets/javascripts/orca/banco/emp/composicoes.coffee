      ...
      jQuery('.select_item').click ->
        ...
        if classe == "insumo" && escolhido.attr('data-tipo') == "1"
          ...
          jQuery('.item_para_timpo_fixo').each ->
            html = html + "<option value='#{jQuery(this).attr("data-codigo")}'>#{jQuery(this).attr("data-codigo")} - #{jQuery(this).attr("data-descricao")}</option>"
          html = html + "</select>
                      </span>
                    </div>
                  </div>"
...
SalvarNew = (form) ->
  ...
  if ValidarDadosDoNovoItem(id, quantidade, operativa, improdutiva, classe, tipo)
    ...
    dados = {id: ComposicaoId(), id_do_item: id, banco:  banco, quantidade: quantidade, operativa: operativa, improdutiva: improdutiva, classe: classe, tipo: tipo, id_item_tempo_fixo: id_item_tempo_fixo}
